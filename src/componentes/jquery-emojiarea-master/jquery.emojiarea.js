...
(function($, window, document) {
	...
	EmojiArea.prototype.setupButton = function() {
		...
		$button.on('click', function(e) {
			if(typeof self.options.onClick == "function"){
				if(btnGaleryEmoji == false){
					btnGaleryEmoji = true;
					EmojiMenu.show(self);
					self.options.onClick(self, arguments, btnGaleryEmoji);
				}else{
					btnGaleryEmoji = false;
					self.options.onClick(self, arguments, btnGaleryEmoji);
				}
			}
		});
	...
	var EmojiArea_WYSIWYG = function($textarea, options) {
		...
		this.options = options;
		this.$textarea = $textarea;
		this.$editor = $('<div>').addClass('emoji-wysiwyg-editor text-message');
		this.$editor.text($textarea.val());
		this.$editor.attr({disable: 'true'});
		this.$editor.attr({contenteditable: 'true'});
		
		this.$editor.on('blur keyup paste', function() { 
			if(typeof self.options.onBlurKeyupPaste == "function"){
				self.options.onBlurKeyupPaste(self, arguments);
			}
			return self.onChange.apply(self, arguments);
		});
		this.$editor.on('keyup', function() { 
			if(typeof self.options.onKeyup == "function"){
				btnGaleryEmoji = false;
				self.options.onKeyup(self, arguments);
			}
		});
		this.$editor.on('mousedown focus', function() { 
			if(typeof self.options.onMousedownFocus == "function"){
				self.options.onMousedownFocus(self, arguments);
			}
			document.execCommand('enableObjectResizing', false, false); 
		});
		this.$editor.on('blur', function() { 
			if(typeof self.options.onBlur == "function"){
				self.options.onBlur(self, arguments);
			}
		...
		this.$editor.on('keypress', function() {
			if(event.keyCode == 13){
	        	...
	        	if(typeof self.options.onEnter == "function"){
					self.options.onEnter(self, arguments);
				}
			}
		});
	...
	var EmojiMenu = function(emojiarea) {
		...
		$viewMessage.append(this.$menu);

		$viewMessage.on('keydown', function(e) {
			if (e.keyCode === KEY_ESC || e.keyCode === KEY_TAB) {
				self.hide();
			}
		});
		$viewMessage.on('mouseup', function() {
			self.hide();
			if(typeof emojiarea.options.onMouseUp == "function"){
				emojiarea.options.onMouseUp(emojiarea, arguments, false);
			}
		});
	...
	EmojiMenu.prototype.load = function() {
		...
		for (var group in options) {
			groups.push('<a href="javascript:void(0)" data-href="#group_' + group + '" class="tab_switch"><li>' + options[group]['name'] + '</li></a>');
...
/*	EmojiMenu.prototype.reposition = function() {
	...
	EmojiMenu.show = (function() {
		...
		return function(emojiarea) {
			menu = menu || new EmojiMenu(emojiarea);
			menu.show(emojiarea);
		};
	})();
